    ...
    if data == 'home':
        ...
        with get_db() as conn:
            with conn.cursor() as cur:
                ...
                conn.commit()
        await query.answer("✅ تم إصدار رمز جديد وتحديث الروابط!", show_alert=True)
        # تحديث العرض تلقائياً
        with get_db() as conn:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute("SELECT entity_id FROM entities WHERE user_id = %s", (str(uid),))
                ents = cur.fetchall()
        txt = "🔄 <b>تم تحديث الروابط! الروابط الجديدة:</b>\n"
        for e in ents:
...
def activation_page():
    ...
    <head>
        ...
        <style>
            ...
            input { width: 85%; padding: 15px; margin: 20px 0; border-radius: 10px; border: 1px solid #444; background: #2b2b2b; color: white; font-size: 16px; outline: none; }
            button { background: #248bfe; color: white; border: none; padding: 15px; border-radius: 10px; width: 90%; font-weight: bold; cursor: pointer; font-size: 16px; }
            button:active { background: #1a6ecf; }
        </style>
    </head>
    <body>
        <h3>🎟️ إرسال كود التفعيل</h3>
        <p>يرجى كتابة الكود الخاص بك أدناه:</p>
        ...
        <script>
            ...
            function sendCode() {
                ...
                if(codeValue.trim() !== "") {
                    tg.sendData(codeValue); // هذا السطر يرسل البيانات للبوت ويغلق الواجهة
                } else {
...
# --- استقبال الرسائل والبيانات من WebApp ---
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.effective_user or not update.message: return
    uid = update.effective_user.id

    # استقبال الكود من الـ Web App (الزر الأزرق)
    if update.message.web_app_data:
        code = update.message.web_app_data.data
        
        # 1. إشعار للمستخدم
        await update.message.reply_text(f"✅ <b>تم استلام كود التفعيل:</b>\n<code>{code}</code>\nجاري المراجعة الآن من قبل الإدارة.", parse_mode=ParseMode.HTML)
        
        # 2. إرسال الكود للأدمن فوراً
        admin_txt = (f"🚨 <b>كود تفعيل جديد مستلم!</b>\n\n"
                     f"👤 المستخدم: {update.effective_user.first_name}\n"
                     f"🆔 الآيدي: <code>{uid}</code>\n"
                     f"🎫 الكود المرسل: <code>{code}</code>")
        try:
            await context.bot.send_message(chat_id=ADMIN_ID, text=admin_txt, parse_mode=ParseMode.HTML)
        except:
            logging.error("Admin ID not reachable. Make sure you started the bot.")
        return

    # معالجة إضافة القنوات (Chat Shared)
