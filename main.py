...
import subscription
import webhooks  
import web_server # استيراد ملف السيرفر اللي فصلناه
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith('set_lang_'):
        ...
        await query.edit_message_text("📥 <b>أدخل كود التفعيل الآن:</b>", parse_mode=ParseMode.HTML, reply_markup=keyboards.get_back_to_home())
    elif data == 'view_chs':
        ...
        await query.answer("🗑️ تم حذف الربط", show_alert=True)
        await check_activation_logic(update, context)
    elif data == 'add_ch':
        ...
        if int(uid) == ADMIN_ID:
            await admin.handle_callback_logic(update, context, data) # دالة موحدة للادمن

# --- 3. معالجة الرسائل وربط القنوات ---
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # استخدام دالة التفعيل من خدماتنا
...
if __name__ == "__main__":
    ...
    # تشغيل سيرفر الويب في الخلفية (Render Port Binding)
    ...
    print("🚀 Bot Sumou Al Arqam is Online & Web Server Started!")
