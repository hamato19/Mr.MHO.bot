...
import config, keyboards, services, init_db, security, owner, admin, terms, subscription
...
# --- 2. معالج الأزرار (الدماغ المحرك لكل النظام) ---
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- أ. أزرار المستخدم العادي ---
    if data.startswith('set_lang_'):
        context.user_data['selected_lang'] = data.split('_')[2]
        await terms.send_terms(update, context, user_lang=context.user_data['selected_lang'])
        ...
        user = services.get_user_data(uid)
        status_msg = f"💳 <b>بيانات الاشتراك:</b>\n\n📅 الانضمام: <code>{user.get('created_at', 'غير مسجل')}</code>\n⏳ الانتهاء: <code>{user.get('expiry_date', 'غير محدد')}</code>\n\n📥 <b>أدخل كود التفعيل (MHO-xxxx) الآن:</b>"
    ...
    # --- ب. أزرار الإدارة (للمالك فقط) ---
        ...
        elif data in ['admin_generate_code', 'admin_gen_codes']: await admin.show_generate_code_menu(update)
        
        # إدارة مستخدم معين
        elif data.startswith('manage_'):
            await admin.manage_single_user(update, context, data.replace('manage_', ''))
            
        # توليد الأكواد (أيام)
        elif data.startswith('gen_days_'):
            await admin.process_generate_code(update, int(data.split('_')[-1]))
            
        # أوامر الحظر والتفعيل (adm_block_id / adm_unblock_id)
...
def tv_webhook(token, target_id):
    # استقبال البيانات كـ Raw Text لضمان عدم ضياع أي حرف من الإشارة
...
# --- 5. التحقق من حالة الاشتراك (Logic) ---
async def check_activation_logic(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    else: await update.effective_chat.send_message(msg, reply_markup=kb, parse_mode=ParseMode.HTML)
