...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == 'ren':
        ...
        await query.edit_message_text(
            "🔄 <b>تجديد أو تفعيل الاشتراك:</b>\n\n• يمكنك الاشتراك عبر موقعنا.\n• أو أرسل <b>كود التفعيل</b> هنا مباشرة.\n\n⏳ <i>البوت في انتظار الكود...</i>", 
            parse_mode='HTML', reply_markup=sub_markup
        )
        return

    # --- 2. لوحة الأدمن (كاملة الخصائص) ---
    if is_owner:
        if data == 'adm':
    ...
    if is_owner or (user and user.get('is_activated')):
        ...
        elif data == 'wh': # الويب هوك (إرسال رسالة قابلة للتنظيف)
            webhook_text = services.format_webhook_links(uid)
            msg = await context.bot.send_message(chat_id=uid, text=f"🌐 <b>روابط الويب هوك الخاصة بك:</b>\n\n<code>{webhook_text}</code>", parse_mode='HTML')
            context.user_data['temp_msg_ids'].append(msg.message_id)

        elif data == 'tok': # توليد التوكن
            new_token = secrets.token_hex(8).upper()
            database.update_user_token(uid, new_token)
            webhook_text = services.format_webhook_links(uid)
            msg = await context.bot.send_message(chat_id=uid, text=f"🔐 <b>تم تحديث رمز الأمان بنجاح!</b>\n\n<code>{webhook_text}</code>", parse_mode='HTML')
            context.user_data['temp_msg_ids'].append(msg.message_id)
...
async def main():
    ...
    logger.info("🚀 سمو الأرقام تعمل بكامل طاقتها ونظام التنظيف الذكي مفعل...")
