...
ADMIN_ID = 8711658382
DOMAIN = "https://your-domain.com" # رابط سيرفرك
    ...
    try:
        with conn.cursor() as cur:
            if state == 'wait_ch':
                ...
                await update.message.reply_text("✅ تم حفظ Key ID.")
            elif state == 'wait_sec':
                cur.execute("UPDATE users SET alpaca_secret_key = %s WHERE user_id = %s", (text, uid)); conn.commit()
                await update.message.reply_text("✅ تم حفظ Secret Key.")
    finally:
        release_db_conn(conn); context.user_data['state'] = None
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'acc':
        await query.edit_message_text(f"👤 <b>بياناتك:</b>\n🆔: <code>{uid}</code>\n🔑: <code>{user['secret_token']}</code>", parse_mode=ParseMode.HTML, reply_markup=await get_main_menu())
        ...
        try:
            ...
            if not ents: await query.edit_message_text("❌ لا توجد قنوات لحذفها.")
                ...
                await query.edit_message_text("❌ <b>اختر القناة التي تريد إزالتها:</b>", reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.HTML)
        ...
        try:
            with conn.cursor() as cur:
                ...
                conn.commit()
            await query.edit_message_text(f"✅ تم إزالة القناة <code>{target_id}</code> بنجاح.", parse_mode=ParseMode.HTML, reply_markup=await get_main_menu())
        finally: release_db_conn(conn)
        ...
        await query.edit_message_text("📢 أرسل ID القناة:"); context.user_data['state'] = 'wait_ch'

    elif query.data == 'gen_token':
        new_t = secrets.token_hex(8); conn = get_db_conn()
        try:
            with conn.cursor() as cur: cur.execute("UPDATE users SET secret_token = %s WHERE user_id = %s", (new_t, uid)); conn.commit()
        ...
        await query.edit_message_text("🚀 إعدادات Alpaca:", reply_markup=InlineKeyboardMarkup(kb))

    elif query.data in ['set_k', 'set_s']:
        await query.edit_message_text("📝 أرسل القيمة:"); context.user_data['state'] = 'wait_key' if query.data == 'set_k' else 'wait_sec'
