...
return
    if text.upper().startswith("SMO-"):
        ...
        # استدعاء ملف المعالج للتحقق من الكود وتفعيل المستخدم
        success, response_text = activation_handler.process_activation(uid, text)
        
        if success:
            # في حال نجاح التفعيل، يتم تعديل الرسالة وإظهار القائمة الرئيسية
            await status_msg.edit_text(f"🎊 {response_text}", parse_mode='HTML')
            await clean_and_show_menu(update, context, uid)
        else:
            # في حال فشل الكود (خاطئ أو مستخدم مسبقاً)
            await status_msg.edit_text(response_text, parse_mode='HTML')
        return

    # الرد الافتراضي إذا لم يكن المدخل كوداً أو أمراً
